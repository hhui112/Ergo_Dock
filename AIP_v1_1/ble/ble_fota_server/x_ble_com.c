 ...
 * 3.5 测试信息：查询 02 06 12 B6 / 12 B7；应答 21 06 12 B4 + 唤醒/响应各 LE16 + 打鼾干预触发 1B + 打鼾包总数 1B + B3 + 24×uint8（cmd 0x21..0x38）。
...
#define BLE_TESTINFO_PARAM_DETAIL   0xB3U  /* 测试信息精确数据（紧挨 B4 汇总之后） */
#define BLE_TESTINFO_TX_DATA_LEN    (2U + 6U + 1U + BLE_TESTINFO_VOICE_DETAIL_N) /* 12 B4 + 2×LE16 + 2×1B + B3 + detail */
...
static uint8_t ble_fill_app_key_data_domain(uint8_t *p) // APP 键值帧/主控盒响应帧（仅填数据域；帧长、端口由 x_ble_com_txbuffFill 写在 p 之前）
{
	uint32_t mt;
	uint32_t keys_le;
	uint8_t raw_len;
...
/*
 * 3.4.3 应答（主控器 -> APP）：数据域固定 14B（帧长 0x0E）。
 * 11 B4 | pwm | tmr | 强度 | 触发 | 剩余 LE32(10ms) | 打鼾包总数 | 1min | 5min | 预留
 * 示例：0E 06 11 B4 32 40 01 01 00 00 00 00 03 02 01 00
 */
...
/* 数据段 0x10：B6 查询 / B7 设置（3.3.2：10 B7 床型） */
...
/*
 * 端口 0x01：普通键 = 键值 4B（小端）+ 保留 1B（5B）；缓启动键值 +Pwm+Tmr（7B）。
 */
...
/* 端口 0x01：3.1 普通/缓启动键值 + 3.2 主控盒响应（非 0x06 数据段-端口模型） */
...
/*
 * 向 APP 主动上报主控盒状态：端口 0x01 + 数据域经 BLE Notification 发出。
 * 由 ble_app_port_inner_dispatch 处理键值后也会调用；键值/数据段应答走各自 notify。
 */
void x_ble_com_txbuffFill(void)
{
	...
	bletxbuff[i++] = 0U;
	bletxbuff[i++] = BLE_APP_PORT_INNER;   // 端口：0x01 = APP 键值帧/主控响应帧
	domain_len = ble_fill_app_key_data_domain(&bletxbuff[2]);
	bletxbuff[0] = domain_len;
	i = 2 + (int)domain_len;
	bletxlen = (uint8_t)i;

#if 1 /* 调试：分包打印 APP 主动上报 notify 内容（量产可改 0 关闭日志） */
	{
		unsigned j, k;
		char line[3U * 16U + 1U];
